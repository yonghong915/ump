...
import com.ump.core.common.aspect.annotation.Dict;
...
//	private ISysDictService dictService;
	...
	// 定义切点Pointcut
	@Pointcut("execution(public * com.ump.*.*.*Ctrler.*(..))")
	 ...
	 * 然后在请求list的时候将这个字典text，已字段名称加_dictText形式返回到前端 例输入当前返回值的就会多出一个sex_dictText字段 {
	 * sex:1, sex_dictText:"男" } 前端直接取值sext_dictText在table里面无需再进行前端的字典转换了
	 * customRender:function (text) { if(text==1){ return "男"; }else if(text==2){
	 * return "女"; }else{ return text; } } 目前vue是这么进行字典渲染到table上的多了就很麻烦了
	 * 这个直接在服务端渲染完成前端可以直接用
	 * 
	 * @param result
	 */
